...
Ensure-WingetPackage -Id 'oss-winget.gperf'     -Exe 'gperf.exe'    -Name 'GNU gperf'
...
try {
    ...
    $westArgs = @("sdk", "install", "-d", $SdkDir)
    if ($env:GITHUB_TOKEN) { $westArgs += @("--personal-access-token", $env:GITHUB_TOKEN) }
    & $West @westArgs
}
finally { Pop-Location }

Info "Full Zephyr workspace ready at: $WS"
